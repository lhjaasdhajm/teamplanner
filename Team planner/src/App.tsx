...
export default function App() {
  ...
  const [selectedDate, setSelectedDate] = useState<Date>(new Date(2026, 2, 9)); // March 9th, 2026
  ...
  const [selectedAssignees, setSelectedAssignees] = useState<Set<string>>(new Set(['Mustafa']));
  const [tempClients, setTempClients] = useState<Set<string>>(new Set(selectedClients));
  const [tempAssignees, setTempAssignees] = useState<Set<string>>(new Set(['Mustafa']));
  ...
  const monitoringFilteredTasks = useMemo(() => {
    if (isMonitoring) {
      ...
      const startMonitoring = new Date(2026, 2, 1); // 1-3-2026
      
      return schedule.filter(t => 
        selectedClients.has(t.client) && 
        selectedAssignees.has(t.assignee) &&
        t.frequency !== 'Dagelijks' &&
        t.date >= startMonitoring &&
        t.date <= today
      );
    }
    return baseFilteredTasks;
  }, [schedule, baseFilteredTasks, isMonitoring, selectedClients, selectedAssignees]);
